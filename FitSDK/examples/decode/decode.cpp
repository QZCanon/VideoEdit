...
public:
    ...
    static void PrintValues(const fit::FieldBase& field)
    {
        for (FIT_UINT8 j=0; j< (FIT_UINT8)field.GetNumValues(); j++)
        {
            ...
            case FIT_BASE_TYPE_FLOAT64:
                ...
                printf("%f",  field.GetFLOAT64Value(j));
                break;
            case FIT_BASE_TYPE_STRING:
                // std::wcout << field.GetSTRINGValue(j);
                printf("%ls",  field.GetSTRINGValue(j).c_str());
                break;
            default:
                break;
            }
    ...
    void OnMesg(fit::Mesg& mesg)
    {
        ...
        printf("   New Mesg: %s.  It has %d field(s) and %d developer field(s).\n", mesg.GetName().c_str(), mesg.GetNumFields(), mesg.GetNumDevFields());

        for (FIT_UINT16 i = 0; i < (FIT_UINT16)mesg.GetNumFields(); i++)
        {
            fit::Field* field = mesg.GetFieldByIndex(i);
            // std::wcout << L"   Field" << i << " (" << field->GetName().c_str() << ") has " << field->GetNumValues() << L" value(s)\n";
            printf("   Field%d(%s) has %d value(s)\n", i,field->GetName().c_str(), field->GetNumValues());
            PrintValues(*field);
        }
        ...
        for (auto devField : mesg.GetDeveloperFields())
        {
            ...
            printf("   Developer Field(%s) has %d  value(s)\n", devField.GetName().c_str(), devField.GetNumValues());
